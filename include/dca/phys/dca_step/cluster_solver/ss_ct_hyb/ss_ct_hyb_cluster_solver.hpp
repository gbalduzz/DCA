...
void SsCtHybClusterSolver<device_t, parameters_type, Data>::collect_measurements() {
  ...
  // sum the sign
  double accumulated_sign = accumulator_.get_accumulated_sign();
  concurrency_.sum_and_average(accumulated_sign);

  // sum G_r_w
  concurrency_.sum_and_average(accumulator_.get_G_r_w());
  accumulator_.get_G_r_w() /= accumulated_sign;

  // sum GS_r_w
  concurrency_.sum_and_average(accumulator_.get_GS_r_w());
